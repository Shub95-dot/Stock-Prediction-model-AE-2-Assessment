...
def _make_dummy_predictions(n: int = 200) -> dict:
    ...
    return {
        m: {h: np.random.randn(n) * 10 + 300 for h in horizons}
        for m in models
    }
...
def _make_dummy_regime(n: int = 200) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "hmm_p0": np.random.rand(n), "hmm_p1": np.random.rand(n),
        "hmm_p2": np.random.rand(n), "hmm_p3": np.random.rand(n),
...
class TestScalerLeakageFix:
    ...
    def setup_method(self):
        self.pipeline = DataPipeline(tickers=[], start="2022-01-01",
                                     end="2024-01-01", window=20)
        df = _make_dummy_df(300)
        self.feature_cols = [c for c in df.columns
                             if c not in {"target_1d","target_5d",
                                          "target_21d","target_63d"}]
        self.df = df
    ...
    def test_pre_fitted_scaler_is_reused(self):
        ...
        np.testing.assert_array_equal(
            returned_scaler.center_, original_center,
    ...
    def test_train_test_scaler_differ_in_center(self):
        ...
        assert not np.allclose(train_scaler.center_, full_scaler.center_), \
...
class TestOOFMetaStacking:
    ...
    def test_meta_predict_after_oof_fit(self):
        ...
        for h in ["t1", "t5", "t21"]:
            ...
            assert np.all((result[h]["up_prob"] >= 0) & (result[h]["up_prob"] <= 1)), \
